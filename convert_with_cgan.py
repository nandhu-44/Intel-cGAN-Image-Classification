...
def process_class_folder(generator, class_folder, conditions, device):
    """Process images in a class folder, splitting into 1/5 for each condition."""
    ...
    print(f"Processing {total_images} images in {class_folder}, split size: {split_size}")
    ...
    for i, image_file in enumerate(image_files):
        ...
        input_path = os.path.join(class_folder, image_file)
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.jpg', dir=class_folder) as temp_file:
...
def main():
    ...
    if not os.path.exists(train_dir) or not os.path.exists(test_dir):
        print("Error: dataset/seg_train/seg_train or dataset/seg_test/seg_test not found.")
        return
    ...
    for split_dir in [train_dir, test_dir]:
        ...
        print(f"Starting processing of {split_name} dataset...")
        for class_name in classes:
            class_folder = os.path.join(split_dir, class_name)
            if not os.path.exists(class_folder):
                print(f"Class folder {class_folder} not found. Skipping.")
                continue
            process_class_folder(generator, class_folder, conditions, device)
        print(f"Completed processing of {split_name} dataset.")
